...
namespace Microsoft.WindowsAPICodePack.Taskbar
{
    ...
    public class TabbedThumbnail : IDisposable
    {
		...
		/// <summary>
		/// The event that occurs when a tab is restored via the taskbar thumbnail preview (context menu).
		/// </summary>
		public event EventHandler<TabbedThumbnailEventArgs> TabbedThumbnailRestored;

		/// <summary>
		/// The event that occurs when a tab is moved via the taskbar thumbnail preview (context menu).
		/// </summary>
		public event EventHandler<TabbedThumbnailEventArgs> TabbedThumbnailMoved;
		...
		internal void OnTabbedThumbnailRestored()
		{
			if ( TabbedThumbnailRestored != null )
				TabbedThumbnailRestored(this, GetTabbedThumbnailEventArgs());
            else
            {
                // No one is listening to these events.
                // Forward the message to the main window
                CoreNativeMethods.SendMessage(ParentWindowHandle, TabbedThumbnailNativeMethods.WM_SYSCOMMAND, new IntPtr(TabbedThumbnailNativeMethods.SC_RESTORE), IntPtr.Zero);
            }
		}
