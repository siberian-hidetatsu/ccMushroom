        ...
        internal bool DispatchMessage(ref Message m, TaskbarWindow taskbarWindow)
        {
            ...
            if (taskbarWindow.EnableTabbedThumbnails)
            {
                ...
                else if (m.Msg == (int)TabbedThumbnailNativeMethods.WM_SYSCOMMAND)
                {
					...
					else if (((int)m.WParam) == TabbedThumbnailNativeMethods.SC_RESTORE)
					{
						// Raise the event
						taskbarWindow.TabbedThumbnail.OnTabbedThumbnailRestored();
					}
					else if (((int)m.WParam) == TabbedThumbnailNativeMethods.SC_MOVE)
					{
						// Raise the event
						taskbarWindow.TabbedThumbnail.OnTabbedThumbnailMoved();
					}
					else if (((int)m.WParam) == TabbedThumbnailNativeMethods.SC_SIZE)
					{
					}
