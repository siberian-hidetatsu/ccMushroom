	...
	public sealed class ShellLink : IDisposable
	{
		...
		public string TargetPath
		{
			get
			{
				...
				bool advertisedShortcuts = false;
				...
				if ( isUnicodeEnvironment )
				{
					...
					advertisedShortcuts = string.IsNullOrEmpty(data.cFileName);
				}
				else
				{
				...
				if ( advertisedShortcuts )
				{
					try
					{
						StringBuilder szProductCode = new StringBuilder(50);
						StringBuilder szFeatureId = new StringBuilder(100);
						StringBuilder szComponentCode = new StringBuilder(50);

						UInt32 res = api.MsiGetShortcutTargetW(currentFile, szProductCode, szFeatureId, szComponentCode);
						if ( res == 0 )
						{
							StringBuilder szPathBuf = new StringBuilder(500);
							UInt32 cchBuf = (UInt32)szPathBuf.Capacity;
							api.INSTALLSTATE installState = api.MsiGetComponentPath(szProductCode.ToString(), szComponentCode.ToString(), szPathBuf, ref cchBuf);
							if ( installState == api.INSTALLSTATE.INSTALLSTATE_LOCAL )
							{
								targetPath = szPathBuf;
							}
						}
					}
					catch ( Exception exp )
